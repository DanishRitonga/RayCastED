...
class WeightedTissueSampler(WeightedRandomSampler):
    def __init__(self, tissues: NDArray[np.uint8], gamma: float = 0.85, replacement: bool = True) -> None:
    ...
    @staticmethod
    def get_sampling_weights_tissue(tissues: NDArray[np.uint8], gamma: float = 1) -> NDArray[np.float64]:
