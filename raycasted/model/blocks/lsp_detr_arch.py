...
class STAttention(nn.Module):
    ...
    def tile(self, x: Tensor, height: int, tile: int) -> tuple[Tensor, int, int]:
        ...
        h, w = x.shape[1:3]
        x = rearrange(
            x, 'b (n_h ts_h) (n_w ts_w) (h d) -> b h (n_h n_w ts_h ts_w) d', ts_h=tile, ts_w=tile, h=self.num_heads
        )
        return x, h, w
...
class Layer(nn.Module):
    def __init__(
        self, dim: int, src_dim: int, num_heads: int, self_sta: dict[str, int], cross_sta: dict[str, int]
    ) -> None:
        super().__init__()
...
class LSPTransformer(nn.Module):
    def __init__(
        ...
        self.radial_distances_head = nn.ModuleList([MLP(dim, dim, num_radial_distances, 3) for _ in feature_levels])
        self.init_weights()
