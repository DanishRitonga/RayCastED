...
- **Val losses persistently NaN (resolved)**: Three-part fix. (1) `overlaps` tensor in `RayCastAssigner.get_box_metrics` forced to float32 — the parent `TaskAlignedAssigner._forward` normalisation uses `self.eps=1e-9` which underflows to 0 in float16, causing NaN that propagates to all 5 loss terms via `target_scores`. (`raycasted/model/tal.py:165`) (2) `polar_iou_torch` and `polar_iou_pairwise_flat_torch` cast inputs to float32 — `POLAR_IOU_EPS=1e-7` also underflows in float16. (`raycasted/data/etl/ops/iou.py`) (3) Loss inputs cast to `.float()` for BCE, Huber, L1, smoothness as belt-and-suspenders. (`raycasted/model/loss.py:152-168`) Training was unaffected because AMP autocast keeps loss in float32.
- **Centroid collapse (XY loss too weak)**: `Huber(delta=0.01)` on normalised [0,1] coordinates produced tiny gradients — a 10px centroid error on a 256px image gave loss=0.0003, which was 4000× smaller than the polar IoU loss. The model learned to predict shapes well but ignored centroid accuracy, causing near-zero mask IoU despite decent polar IoU (mAP@0.5=0.57 polar vs 0.0003 mask). Fix: increase `lambda_xy` from 1.0 to 50.0 and change `delta` from 0.01 to 1.0. (`raycasted/model/loss.py:55,175`)
