...
- **Val losses persistently NaN (resolved)**: Two causes. (1) XY decode mismatch — training decode `(anchor + sigmoid) * stride` produced garbage xy, propagating NaN through polar IoU. Fixed in `loss.py:99`. (2) AMP float16 during validation — `model.half()` makes predictions float16; `POLAR_IOU_EPS=1e-7` underflows to 0 in float16, causing division by zero. Training was unaffected because AMP autocast keeps loss in float32. Fix: cast loss inputs to `.float()` before polar IoU, BCE, Huber, and smoothness computations. (`raycasted/model/loss.py:152-178`)
