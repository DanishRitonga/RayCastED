...
- **`plot_images` / `plot_predictions` crash**: Ultralytics' `plot_images()` and `plot_predictions()` expect 4-dim bboxes but get 34-dim polygon data, crashing on `xywh2xyxy()` / `xyxy2xywh()`. Override both `plot_training_labels()` in `RayCastTrainer` and `plot_predictions()` in `RayCastValidator` as no-ops. (`raycasted/model/train.py:149`, `raycasted/model/val.py:348`)
